...
Solução: Ao encontrar e deleter o primeiro registro par, o sistema saltava o proximo assim deixando alguns itens que deveriam ser excluidos para trás, adicionei um else na condição para que mesmo após a exclusão o indice continuasse parado até a proxima verificação.
...
Solução: Adicionado tratamento de erro com try finally e except no envio e no recebimento do arquivo para capturar erros e liberar os objetos da memoria, criado campo id para renomear o arquivo que agora é enviado um a um e ao enviar o o cds é limpo evitando que seja sobrecarregado e estoure o erro de memoria insuficiente
...
Solução: Adicionado tratamento de erro com try finally e except no envio e no recebimento do arquivo para capturar erros e liberar os objetos da memoria, ao capturar o erro implantado é acionado o método ExcluirArquivos que captura o ultimo index do loop e com isso faz o loop inverso apagando os arquivos.